...
function normalizeExamRecord(input, { fallbackId = '', fallbackCreatedAt = Date.now() } = {}) {
  ...
  return {
    ...
    proctoring: {
      ...
      deductRightClick: sanitizeBoolean(input?.proctoring?.deductRightClick, true),
      deductRestrictedKey: sanitizeBoolean(input?.proctoring?.deductRestrictedKey, true),
    },
...
function defaultGeneralExam() {
  ...
  return {
    ...
    proctoring: {
      ...
      deductRightClick: true,
      deductRestrictedKey: true,
    },
    isDefault: true,
    createdAt: now,
    updatedAt: now,
